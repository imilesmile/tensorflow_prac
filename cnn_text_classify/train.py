    ...
    with sess.as_default():
        ...
        for g, v in grads_and_vars:
            if g is not None:
                '''
                summary有以下几种： 
                1. tf.scalar_summary用于记录数据，tf.image_summary用于记录图片，tf.histogram_summary则会计算某个数据的分布，  
                2. tf.nn.zero_fraction和tf.scalar_summary配合使用，记录矩阵0元素，
                3. tf.merge_summary和tf.merge_all_summaries用于归集summary
                '''
